...
% imdb.images.vind = imdb.images.vind(I);

nViews = sum(imdb.images.sid==1);
imdb.images.id = 1:length(imdb.images.name);

for i=1:length(imdb.images.id)
    viewname=imdb.images.name{i}(1:end-7);
    load([imdb.imageDir '\' viewname '.mat']);
    %     goodview=rank_v(1:nViews);
    viewnum=rem(i,nViews);
    if viewnum~=0
        imdb.images.vind(i) =rank_v(viewnum);
    else
        imdb.images.vind(i) =rank_v(nViews);
    end
end
end
...
for ci = 1:length(imdb.meta.classes),
    ...
    class = [class ci*ones(1,nTrainImages)];
    ...
    if opts.ratio(2)>0,
        inds = (imdb.images.set==1 & class==ci);
