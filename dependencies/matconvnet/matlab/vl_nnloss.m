...
% softmaxmulti added by Ran Song
...
labelSize = [size(X,1) size(X,2) size(X,3) size(X,4)] ;
...
if nargin <= 2 || isempty(dzdy)
    switch lower(opts.loss)
        ...
        case 'hinge'
            t = max(0, 1 - c.*X) ;
        case 'softmaxlogmulti'
            cc=repmat(c./numview,[1 1 1 numview]);
            Xmax = max(X,[],3) ;
            ex = exp(bsxfun(@minus, X, Xmax)) ;
            ...
            t = Xmax + log(sum(ex,3)) - XX ;
            t=reshape(t,size(t,1),size(t,2),size(t,3)*numview,size(t,4)/numview);
            t=sum(t,3);
    end
    %      Y = instanceWeights(:)' * t(:) ;
    Y = t(:) ;
    ...
    switch lower(opts.loss)
        ...
        case 'softmaxlogmulti'
            ...
            yy = bsxfun(@rdivide, ex, sum(ex,3));
            
            yy = bsxfun(@times, dzdy, yy) ;
            tt=reshape(yy,size(yy,1),size(yy,2),size(yy,3),numview,size(yy,4)/numview);
            
            yy(logical(cc))=cc(logical(cc));
            yyt = bsxfun(@times, dzdy, (yy-cc)./(size(c,3)*numview)) ;
            tt=sum(tt,4);
            Y{2}=reshape(tt,size(tt,1),size(tt,2),size(tt,3),size(c,4));
