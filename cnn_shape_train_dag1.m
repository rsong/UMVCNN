function [net,stats] = cnn_shape_train_dag1(net, imdb, getBatch, varargin)
...
%     trainerror=[trainerror stats.train(epoch).error];
%     valerror=[valerror stats.val(epoch).error];
  else
...
%       err = error_multiclass(la, scores);
%       error=sum([error,err],2);
    else
      net.mode = 'val' ;
      net.accumulateParamDers = (s ~= 1) ;
%       net.conserveMemory=0;
...
%   fprintf(' top1error is %.3f',aerr(1)./num);
%   fprintf(' top5error is %.3f',aerr(2)./num);
  fprintf('\n') ;
end
% aerr=aerr./num;
% stats.error=aerr;
...
function err = error_multiclass(labels, scores)
% -------------------------------------------------------------------------

[~,predictions] = sort(scores, 3, 'descend') ;
