...
net = cnn_initialise(imdb.meta.classes, ...
    ...
    'restart', opts.fromScratch, ...
    'nViews', nViews, ...
    'networkType', opts.networkType);%'viewpoolPos', opts.viewpoolPos, ...
load('average_im2400.mat');
net.meta.normalization.averageImage=average_im;
net.meta.trainOpts.learningRate(21)=net.meta.trainOpts.learningRate(20);
% -------------------------------------------------------------------------
%                                                                    Learn
% -------------------------------------------------------------------------
...
if nViews>1
    ...
    % lr = arrayfun(@(l) l.learningRate, net.layers(trainable_layers),'UniformOutput',false);
...
% -------------------------------------------------------------------------
function [im,viewinds] = getSimpleNNBatch(opts, imdb, batch)
% -------------------------------------------------------------------------
...
if nargout > 1, viewinds = imdb.images.vind(batch); end
...
if nargout > 1, viewinds = repmat(viewinds(:)',[1 nAugs]); end
